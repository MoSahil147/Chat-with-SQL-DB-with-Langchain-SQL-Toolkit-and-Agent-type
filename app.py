...
if not api_key:
    ...
    st.stop()  #  Added stop to prevent GroqError
...
def configure_db(db_uri, mysql_host=None, mysql_user=None, mysql_password=None, mysql_db=None):
    if db_uri == LOCALDB:
        ...
        print(dbfilepath)
        creator = lambda: sqlite3.connect(f"file:{dbfilepath}?mode=ro", uri=True)  # corrected 'splite3' to 'sqlite3'
        return SQLDatabase(create_engine("sqlite:///", creator=creator))
    elif db_uri == MYSQL:
        if not (mysql_host and mysql_user and mysql_password and mysql_db):
            st.error("Please provide all MySQL connection details.")
            st.stop()
            ## This is the way how we connect to mysql database
...
## Toolkit, from this we will create query, that will interact with the database
...
## Session state to mention chat history
